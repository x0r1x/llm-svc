...
6. **security** - Настройки безопасности и аутентификации
   - enabled: Включить/выключить аутентификацию по API ключу
   - api_key: API ключ для аутентификации (если null, аутентификация отключена). Может быть задан через переменную окружения API_KEY
   - api_key_header: Название заголовка для API ключа (по умолчанию "X-API-Key")
...
Сервис поддерживает аутентификацию по API ключу, которая может быть включена или отключена в конфигурации.

#### Включение аутентификации

Для включения аутентификации установите параметр `security.enabled: true` и укажите API ключ в параметре `security.api_key`:
...
При включенной аутентификации все запросы к защищенным эндпоинтам должны содержать заголовок `X-API-Key` с валидным ключом:

```bash
curl -X POST http://localhost:8000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "X-API-Key: ваш_секретный_api_ключ" \
  -d '{
...
Для отключения аутентификации установите параметр `security.enabled: false`:
...
При отключенной аутентификации API ключ не требуется для доступа к эндпоинтам.

#### Задание API ключа через переменные окружения

Вместо указания API ключа в конфигурационном файле, вы можете задать его через переменную окружения `API_KEY`:

```bash
export API_KEY=ваш_секретный_api_ключ
```
...
Переменные окружения имеют приоритет над значениями, заданными в конфигурационном файле. Это позволяет легко изменять API ключ для разных окружений без изменения конфигурационных файлов.

Если в конфигурационном файле параметр `security.api_key` установлен в `null` и не задана переменная окружения `API_KEY`, аутентификация будет отключена даже при включенном параметре `security.enabled: true`.

#### Защищенные эндпоинты

Следующие эндпоинты требуют аутентификации при включенной настройке `security.enabled: true`:
